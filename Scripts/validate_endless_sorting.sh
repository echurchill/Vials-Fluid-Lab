...
set -euo pipefail
cd "$(dirname "$0")/.."
output="${TMPDIR:-/tmp}/vials-endless-sorting"
...
xcrun swiftc -O -module-cache-path "$output/module-cache" -parse-as-library \
  ...
  Vials/FluidLab/LabEndlessSorting.swift Vials/FluidLab/LabFluid2D.swift \
  Vials/FluidLab/LabPerformance.swift Vials/FluidLab/LabClassicBoardView.swift \
  Scripts/validate_endless_sorting.swift -o "$output/validate-endless-sorting"
"$output/validate-endless-sorting"
