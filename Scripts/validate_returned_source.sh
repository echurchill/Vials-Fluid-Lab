...
lab_output="${2:-$lab_root/build/returned-source}"
mkdir -p "$lab_output"
...
xcrun swiftc -module-cache-path "$lab_output/ModuleCache" -O -parse-as-library Vials/FluidLab/LabGeometry.swift Vials/FluidLab/LabRenderer.swift \
 ...
 Vials/FluidLab/LabFluid2DView.swift Vials/FluidLab/LabPlanarSurface.swift Scripts/validate_returned_source.swift -o "$lab_output/validate"
