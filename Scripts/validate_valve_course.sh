...
set -euo pipefail
cd "$(dirname "$0")/.."
output="${TMPDIR:-/tmp}/vials-valve-course"
...
xcrun swiftc -O -module-cache-path "$output/module-cache" -parse-as-library \
  ...
  Vials/FluidLab/LabEndlessSorting.swift Vials/FluidLab/LabValveCourse.swift \
  Vials/FluidLab/LabFluid2D.swift Vials/FluidLab/LabPerformance.swift \
  Vials/FluidLab/LabClassicBoardView.swift Scripts/validate_valve_course.swift \
  -o "$output/validate-valve-course"
"$output/validate-valve-course"
