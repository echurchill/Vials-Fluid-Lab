  ...
  for mode in LabBoardPresentation.allCases {
   for puzzle in LabDiscipline.mixing.levels+LabDiscipline.crossover.levels {
    ...
    for (index,operation) in puzzle.authoredRoute(from:state)!.enumerated() {
     ...
     session.reduceMixMotion=puzzle == .coolBlend
     session.activateApparatus(activation.apparatusID,automaticClock:false)
     precondition(session.busy && session.mixing != nil && session.state==state && session.moveCount==0)
     ...
     for tick in 0..<300 where session.busy {
      session.advanceMix(deltaTime:1/60)
      if let mix=session.mixing {
       precondition(session.state==state && session.moveCount==0,"Mix committed prematurely")
       precondition(mix.blend>=0 && mix.blend<=1)
       if session.reduceMixMotion {precondition(mix.agitation==0)}
       if tick==20 {
        let before=mix.time,p=mode == .fluid ? session.renderer!.particleSamples().map(\.position):[]
        let planar=session.fluid2D.particles
        session.togglePause();session.advanceMix(deltaTime:10)
        precondition(session.mixing!.time==before && session.fluid2D.particles==planar)
        if mode == .fluid {precondition(session.renderer!.particleSamples().map(\.position)==p)}
        session.togglePause();session.setSuspended(true);session.advanceMix(deltaTime:10)
        precondition(session.mixing!.time==before);session.setSuspended(false)
       ...
       if index<=3 && [LabBoardPuzzle.warmBlend,.coolBlend,.violetReaction].contains(puzzle),sampled.insert(stage).inserted {
        ...
        if mode == .classic {try capture(LabClassicBoardView(state:session.state,pour:nil,mixing:mix),name)}
     ...
     session.activateApparatus(activation.apparatusID,automaticClock:false)
     session.advanceMix(deltaTime:0.05);session.reset();session.advanceMix(deltaTime:10)
