...
lab_output="${2:-$lab_root/build/concurrent}"
mkdir -p "$lab_output"
...
xcrun swiftc -O -parse-as-library Vials/FluidLab/LabGeometry.swift Vials/FluidLab/LabRenderer.swift \
 ...
 Vials/FluidLab/LabFluid2DView.swift Vials/FluidLab/LabPlanarSurface.swift Scripts/benchmark_concurrent_3d.swift -o "$lab_output/benchmark-3d"
cp "$lab_library" "$lab_output/default.metallib"
"$lab_output/benchmark-3d" "$lab_library" "$lab_output/benchmark.json"
