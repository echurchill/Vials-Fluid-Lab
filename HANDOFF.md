...
The final signed build was installed and launched on Eddie's connected M4 iPad with saved game state intact. Landscape and portrait inspection confirm the helper card remains beside vial summaries and Hint is unobstructed. Connected-device tooling cannot synthesize touches, so direct add/upgrade/pour/Undo acceptance remains for Eddie unless a UI-test target is added.
...
September 24 UI follow-up: the helper add control now appears as the trailing vial-summary card instead of occupying the footer beside Hint. It disappears after two helpers are present. Tea cups and coffee mugs expose their upgrade control on their own summary card, and water jugs do not. Ordinary Classic vial highlights are centered again; only helper hit areas extend to cover a handle. The geometry check and live Mac accessibility flow cover add, both upgrades, maximum-size control removal, second-helper add-card removal and Undo restoration.

## September 24: color-keyed Valve Lab lids (Prototype50)

The provisional cyan line/arrow/FILL marker is replaced by a physical, color-keyed lid in Classic, 2D and 3D. It opens for a matching incoming pour, rejects the wrong pigment with named feedback, and remains visible on empty valves. Redundant lid motifs plus explicit accessibility descriptions keep hue from being the only cue. `LabBoardState` now saves an explicit optional valve pigment per vial; legal moves, exact concurrent commits, solver keys, hints, Undo and completion all use it. Legacy saves infer their keys from existing valve contents or targets. Valve Basics 1 now begins with an empty Tide valve while preserving its original pigment inventory. All fifteen course boards solve, focused empty/wrong/right/save checks pass, presentation captures cover all three modes, and the full concurrency suite passes. See `Reports/Prototype50/README.md`.
